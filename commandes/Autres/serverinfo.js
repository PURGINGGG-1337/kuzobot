module.exports.run = async (client, message, args) => {
    ...
    const db = require("quick.db");
        ...
        ${emojis.online} **Personne en ligne:** ${message.guild.members.cache.filter(m => ["dnd", "idle", "online"].includes(m.user.presence.status)).size}`)//${emojis.off} ${message.guild.members.cache.filter(m => m.user.presence.status === "offline").size}
